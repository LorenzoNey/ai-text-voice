...
/// and falls back to Azure on consecutive errors.
/// </summary>
public class HybridSpeechRecognitionService : ISpeechRecognitionService
{
        ...
        ISettingsService settingsService)
    {
        ...
        _activeService = offlineService; // Start with offline
    ...
    private void OnServiceError(object? sender, RecognitionErrorEventArgs e)
    {
        ...
        // Check if we should fall back to Azure
    ...
    {
        // Initialize offline service (always available)
        await _offlineService.InitializeAsync(language);

        // Configure Azure if settings are available
            ...
            !string.IsNullOrEmpty(settings.AzureRegion))
        {
            ...
            if (settings.UseAzureFallback)
            {
                ...
                {
                    // Azure initialization failure is non-fatal
                    // We'll continue with offline only
                }
            }
        }
    }
    ...
    public async Task<string> StopRecognitionAsync()
    {
        ...
        _consecutiveErrors = 0; // Reset on successful completion
        return result;
    }

    /// <summary>
    /// Switches to Azure Speech Service.
    /// </summary>
    ...
    /// <summary>
    /// Switches back to offline (System.Speech) service.
    /// </summary>
