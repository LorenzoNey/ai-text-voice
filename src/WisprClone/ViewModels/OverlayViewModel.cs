...
public partial class OverlayViewModel : ViewModelBase
{
    ...
    public OverlayViewModel(ISpeechRecognitionService speechService, ISettingsService settingsService)
    {
        ...
        AvailableProviders = new ObservableCollection<ProviderOption>
        {
            new() { Provider = SpeechProvider.Offline, DisplayName = "Offline" },
    ...
    partial void OnSelectedProviderChanged(ProviderOption? value)
    {
        ...
        // ProviderInfo will be updated via StateChanged event from SpeechServiceManager
    }

    /// <summary>
    /// Syncs dropdown selections when settings are changed externally (e.g., from Settings window).
    /// </summary>
    ...
    private void UpdateUIForState(RecognitionState state)
    {
        ...
        {
            RecognitionState.Idle => ("Ready - Press Ctrl+Ctrl", Brushes.Gray, false),
            RecognitionState.Initializing => ("Initializing...", Brushes.Yellow, false),
            RecognitionState.Listening => ("Listening...", Brushes.LimeGreen, true),
            RecognitionState.Processing => ("Processing...", Brushes.Orange, false),
            RecognitionState.Error => ("Error", Brushes.Red, false),
            _ => ("Unknown", Brushes.Gray, false)
        };
