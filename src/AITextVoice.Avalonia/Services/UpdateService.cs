...
public class UpdateService : IUpdateService
{
    ...
    public async Task<string?> DownloadUpdateAsync(IProgress<double>? progress = null, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var tempDir = Path.Combine(Path.GetTempPath(), "AITextVoice-Update");
    ...
    private string? GetPlatformSpecificAssetUrl()
    {
        ...
        {
            // Windows installer is named "AITextVoice-Setup-X.X.X.exe"
            assetPatterns = ["Setup", "Windows-x64"];
        }
        else if (RuntimeInformation.IsOSPlatform(OSPlatform.OSX))
        {
            // macOS DMG is named "AITextVoice-macOS-{arch}-X.X.X.dmg"
        ...
        {
            // Linux AppImage is named "AITextVoice-Linux-x64-X.X.X.AppImage"
    ...
    public void LaunchMacOSUpdate(string dmgPath, Action? onBeforeQuit = null)
    {
        ...
        try
        {
            ...
            var appPath = "/Applications/AITextVoice.app";

            // Create updater script
            var scriptPath = Path.Combine(Path.GetTempPath(), "aitextvoice-update.sh");
            var logPath = Path.Combine(Path.GetTempPath(), "aitextvoice-update.log");
            var script = $@"#!/bin/bash
# AITextVoice macOS Updater
exec > ""{logPath}"" 2>&1
...
if [ -z ""$MOUNT_POINT"" ]; then
    # Try common name
    if [ -d ""/Volumes/AITextVoice"" ]; then
        MOUNT_POINT=""/Volumes/AITextVoice""
    fi
fi
...
if [ -d ""$MOUNT_POINT/AITextVoice.app"" ]; then
    echo ""Copying app to Applications...""
    rm -rf ""{appPath}""
    cp -R ""$MOUNT_POINT/AITextVoice.app"" ""/Applications/""
    echo ""Copy complete""
else
    echo ""ERROR: AITextVoice.app not found in $MOUNT_POINT""
